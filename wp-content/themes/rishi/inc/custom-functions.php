...
if (!function_exists('rishi_setup')) :
	...
	function rishi_setup()
	{
		...
		add_image_size('rishi-fullwidth', 1170, 658, true);
		add_image_size('rishi-withsidebar', 750, 422, true);
		add_image_size('rishi-blog-grid', 350, 197, true);
		add_image_size('rishi-blog-list', 320, 180, true);
		add_image_size('rishi-blog-listing-layout', 360, 563, true);
		add_image_size('rishi-blog-next-prev', 365, 205, true);
...
function rishi_scripts()
{
	...
	$main_bldr_vars = require get_template_directory() . '/js/build/main.asset.php';
	wp_enqueue_script(
		'rara-bldr-main-vars',
		get_template_directory_uri() . '/js/build/main.js',
		array(),
		$main_bldr_vars['version'],
		true
	);

	// Add Translation support for editor JS 
	// wp_set_script_translations( 'rara-bldr-main-vars', 'rishi' );

	wp_localize_script(
		'rara-bldr-main-vars',
		'rt_localizations',
		apply_filters('rt:builder:main:script:vars',[
...
add_action('wp_enqueue_scripts', 'rishi_scripts');
...
if (!function_exists('rishi_admin_scripts')) :
	...
	function rishi_admin_scripts($hook)
	{
		...
		if ( $hook === 'appearance_page_rishi-dashboard' ) {
			...
			$dashboard_assets = require get_template_directory() . '/js/build/dashboard.asset.php';
			wp_enqueue_script(
				'rishi-dashboard',
				get_template_directory_uri() . '/js/build/dashboard.js',
			...
			wp_localize_script(
				...
				array(
					...
					'product_name' 		=> rt_customizer_get_wp_theme()->get('Name'),
			...
			wp_enqueue_style( 'rishi-dashboard-google-fonts', 'https://fonts.googleapis.com/css2?family=Sora:wght@100;200;300;400;500;600;700;800&display=swap', array(), null );
		}
...
function rishi_body_classes($classes)
{
	...
	if (is_page()) {
		...
		$page_content_style_source = rt_customizer_default_akg(
        ...
        if ($page_content_style_source === 'custom') {
            $page_content_area = rt_customizer_default_akg(
                'content_style',
                rt_customizer_get_post_options(),
                'boxed'
            );
	...
	if (is_single()) {
		...
		$post_content_style_source = rt_customizer_default_akg(
            'content_style_source',
            rt_customizer_get_post_options(),
            'inherit'
        );
...
if (!function_exists('rishi_excerpt_length')) :
	...
	function rishi_excerpt_length( $length ){
		...
		$blog_structure = get_theme_mod( $key, rishi_get_default_blogpost_structure() );
...
if (!function_exists('rishi_sanitize_rgba')) :
	/**
	 * Sanitize RGBA colors
	 */
	function rishi_sanitize_rgba($color)
	{
		if ('' === $color) {
			return '';
		}
		...
		if (false === strpos($color, 'rgba')) {
			return rishi_sanitize_hex_color($color);
		}
...
/**
 * RT Customizer Builder additions.
 */

require get_template_directory() . '/rt-customizer-builder/init.php';
