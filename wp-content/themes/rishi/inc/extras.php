...
if (!function_exists('rishi_category')) :
	...
	function rishi_category( $meta ){
		if ('post' === get_post_type()) {
			$categories_list = get_the_category_list(' ');
			if( $categories_list ){
				echo '<span class="cat-links meta-common" data-cat-single="' . esc_attr( $meta['divider'] ) . '" data-cat-style="' . esc_attr( $meta['divider_style'] ) . '">' . $categories_list . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
	}
endif;
	...
	function rishi_post_thumbnail($wrapper = true, $layout = false){
		...
		if (is_singular()) {
			...
			if (is_search() || ((is_archive() || is_home()) && $layout == 'classic')) {
				...
				if ($layout == 'listing') $image_size = 'rishi-blog-list';
...
if ( ! function_exists( 'rishi_single_featured_image' ) ) :
	...
	function rishi_single_featured_image( $prefix, $featured_image_ratio = NULL, $featured_image_size = NULL, $featured_image_visibility = NULL ) {
		...
		if ( ! $featured_image_ratio ) {
			$featured_image_ratio = rt_get_akv_or_customizer(
		...
		if ( ! $featured_image_size ) {
			$featured_image_size = rt_get_akv_or_customizer(
				'featured_image_size',
				$featured_image_source,
				'original'
			);
		...
		$class .= ' ' . rt_customizer_visibility_classes(
			$featured_image_visibility
		);
		...
		return rt_customizer_html_tag('figure', ['class' => $class], rt_customizer_image([
...
if (!function_exists('rishi_breadcrumb')) :
	...
	function rishi_breadcrumb(){
		...
		$separators = [
			'type-1' => function_exists('rt_customizer_image_picker_file') ? rt_customizer_image_picker_file('breadcrumb-sep-1') : '',
			'type-2' => function_exists('rt_customizer_image_picker_file') ? rt_customizer_image_picker_file('breadcrumb-sep-2') : '',
			'type-3' => function_exists('rt_customizer_image_picker_file') ? rt_customizer_image_picker_file('breadcrumb-sep-3') : '',
		];
	...
	function rishi_breadcrumb_start()
	{
		...
		if ($breadcrumbs_position == 'none') {
			...
			if ( ( get_post_type() == 'product' ) && $disable_single_product !== 'yes' ) {
				return;
			} elseif ( is_single() && ( $breadcrumbs_ed_single_post !== 'yes' ) || (rt_customizer_default_akg(
					'breadcrumbs_single_post',
					rt_customizer_get_post_options(),
					'no'
				) === 'yes') ){
				return;
			} elseif ( is_page() && ( $breadcrumbs_ed_single_page !== 'yes' ) || ( ( rt_customizer_default_akg(
					'page_title_hero_section',
					rt_customizer_get_post_options(),
					'default'
				) === 'enabled' ) && ( rt_customizer_default_akg(
		...
		} elseif (is_archive() && $disable_archive !== 'yes') {
			if( rishi_is_woocommerce_activated() && is_shop() && $breadcrumbs_ed_archive_product === 'yes' ){
				rishi_breadcrumb();
			}else{
				return;
			}
...
if (!function_exists('rishi_get_menu_options')) :
	/**
	 * List Navigation Menus
	 */
	function rishi_get_menu_options($key = 'slug')
	{
		$all_menus = get_terms(
			array(
				'taxonomy'   => 'nav_menu',
		...
		$menu_options         = array();
		$menu_options['none'] = esc_html__('None', 'rishi');
...
if (!function_exists('rishi_search_post_count')) :
	...
	function rishi_search_post_count(){
		...
		if( $ed === 'yes' ){
			...
			if ($found_posts > 0) {
				echo '<section class="rishi-search-count"><span class="srch-results-cnt">';
	...
	function rishi_sidebar($class = false)
	{
		...
		if( is_home() ){
			...
			if ($blog_sidebar == 'no-sidebar' || ($blog_sidebar == 'default-sidebar' && $layout == 'no-sidebar')) {
				...
				if ($blog_sidebar == 'right-sidebar' || ($blog_sidebar == 'default-sidebar' && $layout == 'right-sidebar')) $return = $class ? 'rightsidebar' : 'sidebar-1';
				if ($blog_sidebar == 'left-sidebar' || ($blog_sidebar == 'default-sidebar' && $layout == 'left-sidebar')) $return = $class ? 'leftsidebar' : 'sidebar-1';
			} else {
				$return = $class ? 'full-width' : false; //Fullwidth
			}
		}
		...
		if( is_archive() ) {
			...
			if ($archive_sidebar == 'no-sidebar' || ($archive_sidebar == 'default-sidebar' && $layout == 'no-sidebar')) {
				...
				if ($archive_sidebar == 'right-sidebar' || ($archive_sidebar == 'default-sidebar' && $layout == 'right-sidebar')) $return = $class ? 'rightsidebar' : 'sidebar-1';
				if ($archive_sidebar == 'left-sidebar' || ($archive_sidebar == 'default-sidebar' && $layout == 'left-sidebar')) $return = $class ? 'leftsidebar' : 'sidebar-1';
			} else {
				$return = $class ? 'full-width' : false; //Fullwidth
			}
		}
		...
		if( is_search()) {
			...
			if ($search_sidebar == 'no-sidebar' || ($search_sidebar == 'default-sidebar' && $layout == 'no-sidebar')) {
				...
				if ($search_sidebar == 'right-sidebar' || ($search_sidebar == 'default-sidebar' && $layout == 'right-sidebar')) $return = $class ? 'rightsidebar' : 'sidebar-1';
				if ($search_sidebar == 'left-sidebar' || ($search_sidebar == 'default-sidebar' && $layout == 'left-sidebar')) $return = $class ? 'leftsidebar' : 'sidebar-1';
			} else {
				$return = $class ? 'full-width' : false; //Fullwidth
			}
		}
		...
		if( rishi_is_woocommerce_activated() && ( is_shop() || is_product_category() || is_product_tag() || is_singular( 'product' ) ) ) {
			...
			if ($woo_sidebar == 'no-sidebar' || ($woo_sidebar == 'default-sidebar' && $layout == 'no-sidebar')) {
				...
				if ($woo_sidebar == 'right-sidebar' || ($woo_sidebar == 'default-sidebar' && $layout == 'right-sidebar')) $return = $class ? 'rightsidebar' : 'shop-sidebar';
				if ($woo_sidebar == 'left-sidebar' || ($woo_sidebar == 'default-sidebar' && $layout == 'left-sidebar')) $return = $class ? 'leftsidebar' : 'shop-sidebar';
			} else {
				$return = $class ? 'full-width' : false; //Fullwidth
			}
		}
		...
		if(is_singular()) {
			...
			$product_layout = get_theme_mod('single_product_sidebar_layout', $defaults['single_product_sidebar_layout']); //Global Layout/Position for Product        
			...
			$sidebar_layout = rt_customizer_default_akg(
				'page_structure_type',
			...
			if (is_page()) {
				if ($sidebar_layout == 'no-sidebar' || ($sidebar_layout == 'default-sidebar' && $page_layout == 'no-sidebar')) {
					...
					if (($sidebar_layout == 'default-sidebar' && $page_layout == 'right-sidebar') || ($sidebar_layout == 'right-sidebar')) $return = $class ? 'rightsidebar' : 'sidebar-1';
					if (($sidebar_layout == 'default-sidebar' && $page_layout == 'left-sidebar') || ($sidebar_layout == 'left-sidebar')) $return = $class ? 'leftsidebar' : 'sidebar-1';
				} else {
					$return = $class ? 'full-width' : false; //Fullwidth
				}
			}
			...
			if (is_single()) {
				if ('product' === get_post_type()) { //For Product Post Type
					...
					if ($sidebar_layout == 'no-sidebar' || ($sidebar_layout == 'default-sidebar' && $post_layout == 'no-sidebar')) {
						...
						if (($sidebar_layout == 'default-sidebar' && $post_layout == 'right-sidebar') || ($sidebar_layout == 'right-sidebar')) $return = $class ? 'rightsidebar' : 'sidebar-1';
						if (($sidebar_layout == 'default-sidebar' && $post_layout == 'left-sidebar') || ($sidebar_layout == 'left-sidebar')) $return = $class ? 'leftsidebar' : 'sidebar-1';
					} else {
						$return = $class ? 'full-width' : false; //Fullwidth
					}
...
if (! function_exists('rt_customizer_get_woocommerce_ratio')) {
	function rt_customizer_get_woocommerce_ratio() {
		$cropping = get_theme_mod(
			'rt_customizer_woocommerce_thumbnail_cropping',
			'predefined'
		);
...
if (! function_exists('rt_customizer_maybe_get_matching_taxonomy')) {
	function rt_customizer_maybe_get_matching_taxonomy($post_type, $is_category = true) {
