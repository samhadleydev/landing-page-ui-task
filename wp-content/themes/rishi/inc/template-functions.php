...
function rishi_content_start(){ 
	...
	if( is_page() ){
		...
		$streched_ed      = (rt_customizer_default_akg(
	...
	<div class="site-content">
        ...
        */
        ...
        <div class="rishi-container" <?php echo esc_attr( $dataattr ); ?>>
			<div class="main-content-wrapper clear" <?php echo rt_customizer_get_v_spacing(); ?> <?php echo rt_customizer_get_page_spacing(); ?>>
	<?php
}
endif;
...
function rishi_navigation(){
	...
	if( is_singular( 'post' ) && $ed_show_post_navigation === 'yes' && (rt_customizer_default_akg(
				...
				<div class="post-nav-links">
					<?php if( $prev_post ){ ?>
						<div class="nav-holder nav-previous">
							<h2 class="entry-title"><a href="<?php echo esc_url( get_permalink( $prev_post->ID ) ); ?>"><?php echo esc_html( get_the_title( $prev_post->ID ) ); ?></a></h2>
							<div class="meta-nav"><a href="<?php echo esc_url( get_permalink( $prev_post->ID ) ); ?>"><?php esc_html_e( 'Previous', 'rishi' ); ?></a></div>
						</div>
					<?php } if( $next_post ){ ?>
						<div class="nav-holder nav-next">
							<h2 class="entry-title"><a href="<?php echo esc_url( get_permalink( $next_post->ID ) ); ?>"><?php echo esc_html( get_the_title( $next_post->ID ) ); ?></a></h2>
		...
		switch( $pagination ){	
			...
			case 'numbered': // Numbered Pagination
				...
				echo rt_customizer_display_posts_pagination( [ 'pagination_type' => 'simple' ] );
...
function rishi_author(){
	...
	if( get_the_author_meta( 'description' ) && $ed_show_post_author === 'yes'  ){ ?>
...
function rishi_comment(){
	...
	if( ( $ed_single_comment == 'yes' && is_single() ) && ( comments_open() || get_comments_number() ) && (rt_customizer_default_akg(
		'disable_comments',
		rt_customizer_get_post_options(),
		'no'
	) === 'no') ) {
	...
	if( ( $ed_page_comment == 'yes' && is_page() ) && ( comments_open() || get_comments_number() ) && (rt_customizer_default_akg(
		'disable_comments',
		rt_customizer_get_post_options(),
		'no'
	) === 'no') ) {
...
function rishi_archive_heading(){ 
	...
	if( is_archive() ){
		...
		elseif( is_author() ){
			...
			if( get_the_author_meta( 'description' ) || ( $author_page_avatar_ed === 'yes' ) ){ ?>
...
if( ! function_exists( 'rishi_search_page_label' ) ) :
	...
	*/
	function rishi_search_page_label(){ 
		...
		if( $search_page_label ){
		?>
		<span class="search-res"><?php echo esc_html( $search_page_label ); ?></span>
		<?php 
		}
	}
endif;
...
if( ! function_exists( 'rishi_scrolltotop' ) ) :
	...
	switch ( $top_button_type ) {
		...
		case 'type-1':
			$svg_image = rt_customizer_image_picker_file('top-1');
		break;

		case 'type-2':
			$svg_image = rt_customizer_image_picker_file('top-2');
		break;

		case 'type-3':
			$svg_image = rt_customizer_image_picker_file('top-3');
		break;

		case 'type-4':
			$svg_image = rt_customizer_image_picker_file('top-4');
		break;
	...
	$back_top_visibility_class = 'to_top ' . rt_customizer_visibility_classes(
		rt_get_akv_or_customizer(
			'back_top_visibility',
			$atts,
			[
				'desktop' => true,
...
if( ! function_exists( 'rishi_get_related_post_info' ) ) :
	...
	function rishi_get_related_post_info(){
		...
		if ($single_title) echo '<h3 class="blog-single-wid-title"><span>' . esc_html($single_title) . '</span></h3>';
	}
endif;
