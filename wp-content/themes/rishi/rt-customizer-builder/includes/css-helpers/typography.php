...
class RT_Customizer_Fonts_Manager
{
	public function get_all_fonts()
	{
		return apply_filters('rt_customizer_typography_font_sources', [
			'system' => [
				'type' => 'system',
	...
	public function get_dynamic_fonts_ids()
	{
		...
		], Rishi_Manager::instance()->builder->typography_keys());

		$page_title_source = rt_customizer_get_page_title_source();

		if ($page_title_source) {
			$fonts_ids[] = rt_get_akv_or_customizer(
				'pageTitleFont',
				$page_title_source,
				rt_customizer_typography_default_values([
					'size' => [
	...
	public function load_dynamic_google_fonts()
	{
		...
		if (!empty($url)) {
			wp_register_style('rishi-fonts-font-source-google', $url, [], null);
	...
	{
		$saved_data = get_option('rt_customizer_google_fonts', false);
		...
		if (
			...
			$response = wp_remote_get( get_template_directory_uri() . '/rt-customizer-builder/customizer-settings/google-fonts/google-fonts.json'
			);
			...
			if (
				...
				update_option('rt_customizer_google_fonts', [
					'last_update' => time(),
					'fonts' => $body
				], false);
...
if (!function_exists('rt_customizer_output_font_css')) {
	function rt_customizer_output_font_css($args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		if ($args['font_value']['family'] === 'System Default') {
			...
			$fonts_manager = new RT_Customizer_Fonts_Manager();
		...
		$args['css']->put(
			$args['selector'],
			"--" . rt_customizer_camel_case_prefix('fontFamily', $args['prefix']) . ": {$correct_font_family}"
		);

		$weight_and_style = rt_customizer_get_css_for_variation(
			$args['font_value']['variation']
		);

		$args['css']->put(
			$args['selector'],
			"--" . rt_customizer_camel_case_prefix('fontWeight', $args['prefix']) . ": {$weight_and_style['weight']}"
		);

		if ($weight_and_style['style'] !== 'normal') {
			$args['css']->put(
				$args['selector'],
				"--" . rt_customizer_camel_case_prefix('fontStyle', $args['prefix']) . ": {$weight_and_style['style']}"
			);
		}

		$args['css']->put(
			$args['selector'],
			"--" . rt_customizer_camel_case_prefix('textTransform', $args['prefix']) . ": {$args['font_value']['text-transform']}"
		);

		$args['css']->put(
			$args['selector'],
			"--" . rt_customizer_camel_case_prefix('textDecoration', $args['prefix']) . ": {$args['font_value']['text-decoration']}"
		);
		...
		rt_customizer_output_responsive([
			...
			'variableName' => rt_customizer_camel_case_prefix('fontSize', $args['prefix']),
			'unit' => '',
		...
		rt_customizer_output_responsive([
			...
			'mobile_css' => $args['mobile_css'],
			'selector' => $args['selector'],
			'variableName' => rt_customizer_camel_case_prefix(
				'lineHeight',
				$args['prefix']
			),
			'unit' => '',
...
if (!function_exists('rt_customizer_get_css_for_variation')) {
	function rt_customizer_get_css_for_variation($variation, $should_output_normals = true)
	{
		$weight_and_style = [
			'style' => '',
			'weight' => '',
...
if (!function_exists('rt_customizer_typography_default_values')) {
	function rt_customizer_typography_default_values($values = [])
	{
		return array_merge([
			'family' => 'Default',
			'variation' => 'Default',
...
add_action('wp_ajax_rt_customizer_get_fonts_list', function () {
