...
if (!function_exists('rt_customizer_compute_box_shadow_var_for')) {
	function rt_customizer_compute_box_shadow_var_for($value, $should_skip_output = true)
	{
		if (
			!isset($value['enable'])
			||
...
if (!function_exists('rt_customizer_box_shadow_value')) {
	function rt_customizer_box_shadow_value($args = [])
	{
...
if (!function_exists('rt_customizer_output_box_shadow')) {
	function rt_customizer_output_box_shadow($args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		$responsive = rt_customizer_expand_responsive_value($args['responsive'] || true);

		$shadow_value = [
			'desktop' => ($responsive['desktop'] ? rt_customizer_compute_box_shadow_var_for(
				$value['desktop'],
				$args['should_skip_output']
			) : 'none'),

			'tablet' => ($responsive['tablet'] ? rt_customizer_compute_box_shadow_var_for(
				$value['tablet'],
				$args['should_skip_output']
			) : 'none'),

			'mobile' => ($responsive['mobile'] ? rt_customizer_compute_box_shadow_var_for(
				$value['mobile'],
				$args['should_skip_output']
			) : 'none')
		];
