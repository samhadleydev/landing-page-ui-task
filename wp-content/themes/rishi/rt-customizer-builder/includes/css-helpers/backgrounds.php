...
if (!function_exists('rt_customizer_get_patterns_svgs_list')) {
	function rt_customizer_get_patterns_svgs_list()
	{
		return array(
...
if (!function_exists('rt_customizer_get_svg_pattern')) {
	function rt_customizer_get_svg_pattern($name, $color = '#ccc')
	{
		$opacity = 1;
		...
		$patterns = rt_customizer_get_patterns_svgs_list();
...
if (!function_exists('rt_customizer_output_background_css')) {
	function rt_customizer_output_background_css($args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		if (!$args['responsive']) {
			rt_customizer_output_single_background_css([
				'value' => $args['value'],
				'css' => $args['css'],
		...
		rt_customizer_output_single_background_css([
			...
			'css' => $args['css'],
			'selector' => empty($args['desktop_selector_prefix']) ? $args['selector'] : ($args['desktop_selector_prefix'] . ' ' . $args['selector']),
			'important' => $args['important']
		]);

		if (!rt_customizer_values_are_similar(
			...
			rt_customizer_output_single_background_css([
				...
				'css' => $args['tablet_css'],
				'selector' => empty($args['tablet_selector_prefix']) ? $args['selector'] : ($args['tablet_selector_prefix'] . ' ' . $args['selector']),
				'important' => $args['important']
			]);
		}
		...
		if (!rt_customizer_values_are_similar(
			...
			rt_customizer_output_single_background_css([
				...
				'css' => $args['mobile_css'],
				'selector' => empty($args['mobile_selector_prefix']) ? $args['selector'] : ($args['mobile_selector_prefix'] . ' ' . $args['selector']),
				'important' => $args['important']
			]);
		}
	}
...
if (!function_exists('rt_customizer_output_single_background_css')) {
	function rt_customizer_output_single_background_css($args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		$args['value'] = rt_customizer_background_default_value($args['value']);

		$backgroundColor = rt_customizer_get_colors(
			rt_get_akv(
				'backgroundColor',
				$args['value'],
		...
		$overlayColor = rt_customizer_get_colors(
			rt_get_akv(
				'overlayColor',
				$args['value'],
				['default' => ['color' => '#e5e7ea']]
		...
		if ($args['value']['background_type'] === 'image') {
			$background_image = rt_get_akv('background_image', $args['value'], [
				'attachment_id' => null,
				'x' => 0.5,
				'y' => 0.5
			]);
			...
			if (!empty($attachment_url)) {
				...
				$background_position = rt_customizer_maybe_append_important(
					(floatval($background_image['x']) * 100) . "% " . (floatval($background_image['y']) * 100) . '%',
					$args['important']
				);
				...
				$background_size = rt_customizer_maybe_append_important(
					rt_get_akv('background_size', $args['value'], 'auto'),
					$args['important']
				);
				...
				$background_attachment = rt_customizer_maybe_append_important(rt_get_akv(
				...
				$background_repeat = rt_customizer_maybe_append_important(rt_get_akv(
					'background_repeat',
					$args['value'],
					'no-repeat'
				), $args['important']);
		...
		if ($args['value']['background_type'] === 'pattern') {
			$patternColor = rt_customizer_get_colors(
				rt_get_akv(
					'patternColor',
					$args['value'],
					['default' => ['color' => '#e5e7ea']]
			...
			$image_url = 'url("' . rt_customizer_get_svg_pattern(
				rt_get_akv('background_pattern', $args['value'], 'type-1'),
				$patternColor['default']
			) . '")';
		}

		if ($args['value']['background_type'] === 'gradient') {
			$gradient = rt_get_akv('gradient', $args['value'], '');
		...
		$image_url = rt_customizer_maybe_append_important($image_url, $args['important']);
		...
		if ($image_url !== 'none' && $image_url !== 'CT_CSS_SKIP_RULE') {
			$overlayColor['default'] = rt_customizer_maybe_append_important(
			...
			$args['css']->put(
				$args['selector'],
				"--overlay: {$overlayColor['default']}"
			);
		}

		$backgroundColor['default'] = rt_customizer_maybe_append_important(
...
if (!function_exists('rt_customizer_background_default_value')) {
	function rt_customizer_background_default_value($values = [])
	{
