...
if (!function_exists('rt_customizer_post_meta')) {
	function rt_customizer_post_meta($post_meta_descriptor = null, $args = [])
	{
		if (!$post_meta_descriptor) {
			$post_meta_descriptor = rt_customizer_post_meta_defaults([
				[
					'id' => 'author',
					'enabled' => true,
				],
		...
		foreach ($post_meta_descriptor as $index => $single_meta) {
			...
			if (
				...
				$post_meta_descriptor[$index]['label'] = __('By', 'rishi');
			}

			if (
				($single_meta['id'] === 'post_date'
					||
					$single_meta['id'] === 'updated_date') && !isset($single_meta['label'])
			) {
				$post_meta_descriptor[$index]['label'] = __('On', 'rishi');
			}

			if (
				($single_meta['id'] === 'categories'
					||
					$single_meta['id'] === 'tags') && !isset($single_meta['label'])
			) {
				$post_meta_descriptor[$index]['label'] = __('In', 'rishi');
			}
		...
		foreach ($post_meta_descriptor as $single_meta) {
			...
			if ( $single_meta['id'] === 'author' && get_the_author() ){ ?>
				<li class="meta-author" <?php echo rt_customizer_schema_org_definitions('author_name') ?>><?php
					if ($single_meta['has_author_avatar'] === 'yes') {
						echo rt_customizer_simple_image(
							get_avatar_url(
					...
					echo rt_customizer_html_tag('a', array_merge([
						'class' => 'rt-meta-element-author',
						'href' => esc_url(get_author_posts_url($authordata->ID, $authordata->user_nicename)),
						/* translators: %s: Author's display name. */
						'title' => esc_attr(sprintf(__('Posts by %s', 'rishi'), get_the_author())),
						'rel' => 'author',
					], rt_customizer_schema_org_definitions('author_link', [
						'array' => true
					]), ($args['meta_type'] === 'label' ? [
							'data-label' => __('By', 'rishi')
						] : [])), get_the_author());
			...
			if ($single_meta['id'] === 'post_date') { ?>
				<li class="meta-date" <?php echo rt_customizer_schema_org_definitions('publish_date') ?>><?php
					if ($args['meta_type'] === 'icons' || $args['force_icons']) { ?>
						<svg width="13" height="13" viewBox="0 0 15 15">
					...
					echo rt_customizer_html_tag(
						'time',
						array_merge([
							'class' => 'rt-meta-element-date',
							'datetime' => get_the_date('c')
						], (
							($args['meta_type'] === 'label') ? ([
								'data-label' => __('On', 'rishi')
			...
			if ($single_meta['id'] === 'updated_date') { ?>
				<li class="meta-updated-date" <?php echo rt_customizer_schema_org_definitions('modified_date') ?>>
					<?php
						if ($args['meta_type'] === 'icons' || $args['force_icons']) { ?>
							<svg width="13" height="13" viewBox="0 0 15 15">
			...
			if ($single_meta['id'] === 'categories' && rt_customizer_get_categories_list()) {
				...
				echo wp_kses_post(rt_customizer_get_categories_list($divider));

				echo '</li>';
			}

			if ($single_meta['id'] === 'tags' && rt_customizer_get_categories_list('', false)) {
				if (
				...
				if ($args['meta_type'] === 'label') {
					echo '<span>' . esc_html($single_meta['label'], 'rishi') . '</span>';
				}

				echo wp_kses_post(rt_customizer_get_categories_list($divider, false));
		...
		echo '<ul ' . rt_customizer_attr_to_html($container_attr) . ' ' . rt_customizer_schema_org_definitions('blog') . '>';
...
if (!function_exists('rt_customizer_get_categories_list')) {
	function rt_customizer_get_categories_list($between = '', $is_category = true)
	{
		global $post;
...
function rt_customizer_post_meta_defaults($opts = [], $args = [])
{
	$args = wp_parse_args(
		$args,
		[]
