 ...
 */
if (! function_exists('rt_customizer_display_posts_pagination')) {
	function rt_customizer_display_posts_pagination($args = []) {
		global $wp_query;
		...
		$args['prefix'] = rt_customizer_manager()->screen->process_allowed_prefixes(
			$args['prefix'],
			$allowed_prefixes_args
		);
		...
		if (
			...
			$button_output = '<a class="rt-button rt-load-more">' . $label_button . '</a>';
		}
		...
		if (
			...
			$button_output = '<div class="rt-load-more-helper">' . $button_output;
			$button_output .= '<span data-loader="circles"><span></span><span></span><span></span></span>';
			$button_output .= '<div class="rt-last-page-text">' . $lasttext . '</div>';
			$button_output .= '</div>';
		}

		$pagination_class = 'rt-pagination';
		$divider_output = '';
		...
		foreach ($links as $link) {
			...
			if (
				...
				$link = str_replace(
					'page-numbers',
					trim('page-numbers ' . rt_customizer_visibility_classes(
						$arrows_visibility
					)),
					$link
				);
		...
		if ($args['pagination_type'] === 'simple') {
			$proper_links = '<div class="' . rt_customizer_visibility_classes(
				$numbers_visibility
			) . '">' . $proper_links . '</div>';
		}
