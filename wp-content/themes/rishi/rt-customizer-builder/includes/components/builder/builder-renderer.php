...
abstract class Rishi_Builder_Render
{
	...
	public function get_section_value()
	{
		if ($this->get_section_type() === 'header') {
			return rt_customizer_manager()->header_builder->get_section_value();
		}

		return rt_customizer_manager()->footer_builder->get_section_value();
	}
	...
	public function get_current_section_id()
	{
		...
		if ($this->get_section_type() === 'header') {
			return rt_customizer_manager()->header_builder->get_current_section_id();
		}

		return rt_customizer_manager()->footer_builder->get_current_section_id();
	}
	...
	public function get_current_section()
	{
		...
		if ($this->get_section_type() === 'header') {
			return rt_customizer_manager()->header_builder->get_current_section();
		}

		return rt_customizer_manager()->footer_builder->get_current_section();
	}
	...
	public function get_root_selector($item = null)
	{
		...
		if ($item) {
			...
			if ($item['id'] === 'socials') {
				$selector .= '.rt-' . $this->get_section_type() . '-socials';
			}

			if ($item['id'] === 'divider') {
				$selector .= '.ct-' . $this->get_section_type() . '-divider';
			}

			if ($item['id'] === 'contacts') {
				$selector .= '.ct-' . $this->get_section_type() . '-contact-info';
			}
		...
		if (
			...
			if ($this->get_section_type() === 'footer') {
				$header_prefix .= ' footer.rt-footer';
			}
		}
	...
	public function get_item_config_for($id)
	{
		...
		$registered_items = rt_customizer_manager()
			->builder
