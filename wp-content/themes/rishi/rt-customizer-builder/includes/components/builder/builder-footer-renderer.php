...
class Rishi_Footer_Builder_Render extends Rishi_Builder_Render
{
	...
	public function render_row($row)
	{
		...
		if ($count === 2) {
			$columns = rt_customizer_default_akg(
				'2_columns_layout',
				$atts,
				[
		...
		$container_class = 'rt-container';

		if (rt_customizer_default_akg('footerRowWidth', $atts, 'fixed') !== 'fixed') {
		...
		if (
			rt_customizer_default_akg('footerColumnsDivider', $atts, [
				'width' => 1,
				'style' => 'none',
				'color' => [
					'color' => '#dddddd',
		...
		$visibility_classes = rt_customizer_visibility_classes(
		...
		$result .= '<div ' . rt_customizer_attr_to_html($columns_wrapper_attr) . '>';
		...
		foreach ($row['columns'] as $index => $column) {
			...
			if (!empty(trim($items))) {
				$result .= '<div ' . rt_customizer_attr_to_html($column_attr) . '>';
				$result .= $items;
				$result .= '</div>';
			} else {
