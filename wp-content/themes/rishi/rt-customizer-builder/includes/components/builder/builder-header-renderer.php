...
class Rishi_Header_Builder_Render extends Rishi_Builder_Render
{
	...
	public function get_header_height($sticky_height = false)
	{
		...
		$top_row_height = rt_get_akv('headerRowHeight', $top_row, [
			'mobile' => 50,
			'tablet' => 50,
			'desktop' => 50,
		]);

		$middle_row_height = rt_get_akv('headerRowHeight', $middle_row, [
			'mobile' => 70,
			'tablet' => 70,
			'desktop' => 120,
		]);

		$bottom_row_height = rt_get_akv('headerRowHeight', $bottom_row, [
			'mobile' => 80,
			'tablet' => 80,
			'desktop' => 80,
		]);
		...
		if ($sticky_height) {
			...
			if (
				...
				if (rt_get_akv('has_sticky_shrink', $middle_row, 'no') === 'yes') {
					$middle_row_shrink = rt_customizer_expand_responsive_value(rt_get_akv(
	...
	public function contains_item($item, $is_primary = false)
	{
		...
		if ($is_primary) {
			if ($item === 'offcanvas') {
				return !$this->is_row_empty($item, 'mobile');
			}

			return (!$this->is_row_empty($item, 'desktop')
				||
	...
	public function render_row($row, $args = [])
	{
		...
		$container_class = 'rishi-container';

		if (rt_customizer_default_akg('headerRowWidth', $atts, 'fixed') !== 'fixed') {
			$container_class = 'rt-container-fluid';
		}
		...
		if (in_array('search-input', $middle['items'])) {
			$attr['data-middle'] = 'search-input';
		}

		$row_container_attr = apply_filters(
			'rishi:header:row-wrapper-attr',
			array_merge($attr, (is_customize_preview() ? [
		...
		$result = '<div ' . rt_customizer_attr_to_html($row_container_attr) . '>';
		$result .= '<div ' . rt_customizer_attr_to_html(array_merge([
	...
	public function render_single_item($item_id, $args = [])
	{
		...
		$registered_items = rt_customizer_manager()->builder->get_registered_items_by('header');
		...
		$not_registered_label = sprintf(
			...
			__('Item %s not registered or doesn\'t have a view.php file.', 'rishi'),
			$item_id
		);
		...
		$render_args = apply_filters('rishi:header:item-template-args', [
			'panel_type' => 'header',
		...
		return rt_customizer_render_view(
			apply_filters(
				'rishi:header:item-view-path:' . $item_id,
				$item['path'] . '/view.php'
			),
			$render_args,
	...
	private function render_start_placement($row, $args = [])
	{
		...
		if (count($placement['items']) > 0) {
			$primary_output = rt_customizer_html_tag(
				'div',
				[
					'data-items' => 'primary'
				],
		...
		if (
			...
			$secondary_output = rt_customizer_html_tag(
				'div',
				[
