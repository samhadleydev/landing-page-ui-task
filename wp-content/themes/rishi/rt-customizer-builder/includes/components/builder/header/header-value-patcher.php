...
class Rishi_Header_Builder_Patcher
{
	...
	private function patch_header_placements($value)
	{
		...
		$default = rt_customizer_manager()->header_builder->get_default_value();
		...
		foreach ($default['sections'] as $actual_section) {
			if (strpos($actual_section['id'], 'rt-custom') !== false) {
		...
		if (
			...
			&&
			strpos($value['current_section'], 'rt-custom') !== false
		) {
