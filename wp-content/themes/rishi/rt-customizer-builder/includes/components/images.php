...
if (!function_exists('rt_customizer_has_lazyload')) {
	function rt_customizer_has_lazyload()
	{
		if (
			wp_doing_ajax()
			&&
		...
		return class_exists( 'Rishi\Rishi_Companion' ) && get_theme_mod('has_lazy_load', 'yes') === 'yes';
	}
}
 ...
 */
if (!function_exists('rt_customizer_image')) {
	function rt_customizer_image($args = [])
	{
		$args = wp_parse_args(
			$args,
			[
				...
				'img_atts' => [],
				'inner_content' => '',
				'lazyload' => rt_customizer_has_lazyload(),
			...
			if ($args['no_image_type'] === 'woo') {
				...
				if ($placeholder_image) {
					if (is_numeric($placeholder_image)) {
						...
						return rt_customizer_simple_image($placeholder_image, $args);
					}
				}
			}
		}
		...
		if ($args['ratio_blocks']) {
			$args['inner_content'] .= rt_customizer_generate_ratio(
				$args['ratio'],
				$args['attachment_id'],
				$args['size']
			);
		...
		return '<' . $args['tag_name'] . ' ' . $other_html_atts . '>' .
			rt_customizer_get_image_element($args) .
			$args['inner_content'] .
			'</' . $args['tag_name'] . '>';
	}
}
 ...
 * @param array $args various params that the function accepts.
 */
if (!function_exists('rt_customizer_get_image_element')) {
	function rt_customizer_get_image_element($args)
	{
		if (!wp_get_attachment_image_src($args['attachment_id'])) {
			return '';
		}
 ...
 */
if (!function_exists('rt_customizer_generate_ratio')) {
	function rt_customizer_generate_ratio($ratio, $attachment_id = null, $size = null)
	{
		$result = 0;
		...
		if ('original' === $ratio) {
			if (!$attachment_id) {
				...
				$all_sizes = rt_customizer_get_all_wp_image_sizes();
...
if (!function_exists('rt_customizer_get_all_wp_image_sizes')) {
	function rt_customizer_get_all_wp_image_sizes()
	{
		...
		// rt_customizer_print($image_sizes);
 ...
 * @param array $args various params that the function accepts.
 */
if (!function_exists('rt_customizer_simple_image')) {
	function rt_customizer_simple_image($image_src, $args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		if ($args['ratio_blocks']) {
			$args['inner_content'] .= rt_customizer_generate_ratio($args['ratio']);
		}
