...
if (!function_exists('rt_customizer_social_icons')) {
	function rt_customizer_social_icons($socials_descriptor = null, $args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		return rt_customizer_get_social_box([
			'attr' => $attr,
 ...
 */
if (!function_exists('rt_customizer_get_social_share_box')) {
	function rt_customizer_get_social_share_box($args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		if (
			...
			$before_content = rt_customizer_html_tag(
				'span',
				[
...
if (!function_exists('rt_customizer_get_dynamic_social_networks')) {
	function rt_customizer_get_dynamic_social_networks()
	{
		...
		foreach ($networks as $network) {
			rt_customizer_assert_args($network, ['id', 'name', 'icon']);
		}
...
if (!function_exists('rt_customizer_get_social_metadata')) {
	function rt_customizer_get_social_metadata($args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		foreach (rt_customizer_get_dynamic_social_networks() as $dynamic_network) {
		...
		if ($args['type'] === 'share') {
			$home_url = rt_customizer_encode_uri_component(
				get_the_permalink()
			);

			if (!empty($args['custom_url'])) {
				$home_url = rt_customizer_encode_uri_component($args['custom_url']);
			}
			...
			if (isset($social_urls[$args['social']])) {
				$single_metadata['url'] = str_replace(
					...
					str_replace(
						'{text}',
						rt_customizer_encode_uri_component(get_the_title()),
...
if (!function_exists('rt_customizer_get_social_share_items')) {
	function rt_customizer_get_social_share_items()
	{
		$prefix = rt_customizer_manager()->screen->get_prefix();
...
if (!function_exists('rt_customizer_get_social_box')) {
	function rt_customizer_get_social_box($args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		if ($args['type'] === 'share') {
			$args['socials'] = rt_customizer_get_social_share_items();
		}

		if ($args['socials'] === null) {
			$args['socials'] = [];

			foreach (rt_customizer_get_social_networks_list() as $network_id => $network) {
				$args['socials'][] = [
					'id' => $network_id,
					'enabled' => true
				];
...
?>
		...
		<div <?php echo rt_customizer_attr_to_html($old_attr) ?>>
			...
			if ($args['links_wrapper']) {
				...
				$result = trim($result . ' ' . rt_customizer_attr_to_html(
			...
			<?php foreach ($args['socials'] as $single_social) { ?>
				...
				$metadata = rt_customizer_get_social_metadata([
					'type' => $args['type'],
					'social' => $single_social['id'],
					'custom_url' => $args['custom_share_url']
				]);

				$label_attr = ['class' => 'rt-label'];
				...
				<a href="<?php echo $metadata['url'] ?>" target="_blank" rel="noopener noreferrer" data-network="<?php echo $single_social['id'] ?>" aria-label="<?php echo $metadata['name'] ?>">

					<span class="rt-icon-container">
						<?php echo $metadata['icon'] ?>
					</span>

					<span <?php echo rt_customizer_attr_to_html($label_attr) ?>><?php echo $metadata['name'] ?></span>
				</a>
			<?php } ?>
 ...
 */
if (!function_exists('rt_customizer_encode_uri_component')) {
	function rt_customizer_encode_uri_component($str)
	{
		$revert = [
			'%21' => '!',
			'%2A' => '*',
...
if (!function_exists('rt_customizer_get_social_networks_list')) {
	function rt_customizer_get_social_networks_list()
	{
		...
		$dynamic_networks = rt_customizer_get_dynamic_social_networks();
