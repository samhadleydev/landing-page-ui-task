...
function rt_customizer_get_frontend_selector_for_prefix($prefix = '')
{
...
function rt_customizer_sync_single_post_thumbnail($args = [])
{
	...
	return array_merge([
		...
		'render' => function () use ($args) {
			...
			echo rishi_single_featured_image( 'single_blog_post' );
		}
	], $args);
}

function rt_customizer_replace_current_template()
{
	$tag_templates = array(
	...
	if (
		...
		echo '<main ' . rt_customizer_main_attr() . '>';

		// rt_customizer_before_current_template();
		get_template_part('template-parts/single');
		// rt_customizer_after_current_template();
...
add_filter('customize_dynamic_partial_class', function ($class, $id, $args) {
	return 'Rishi_WP_Customize_Partial';
}, 10, 3);

class Rishi_WP_Customize_Partial extends WP_Customize_Partial
{
