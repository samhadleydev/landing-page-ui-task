...
/**
 * RT Screen manager class
 */
class RT_Screen_Manager
{
	private $prefixes = [];
	...
	private function compute_prefix($args = [])
	{
		...
		if (rt_customizer_is_page([
			'shop_is_page' => false,
			'blog_is_page' => false
		]) || is_single()) {
			if (is_single()) {
				$post_type = rt_customizer_manager()->post_types->is_supported_post_type();
			...
			if (!$actual_prefix) {
				$actual_prefix = rt_customizer_is_page() ? 'single_page' : 'single_blog_post';
			}
		}
		...
		if (
			(is_category()
				...
				is_archive()
				||
				is_post_type_archive()) && !is_author() && !$actual_prefix
		) {
			$post_type = rt_customizer_manager()->post_types->is_supported_post_type();
