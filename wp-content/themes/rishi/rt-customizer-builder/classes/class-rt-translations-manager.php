<?php
//TODO:Check this if required or not before live.
/**
 * RT Translation Manager
 */
class RT_Translations_Manager
{
	public function get_all_translation_keys()
	{
		$builder_keys = Rishi_Manager::instance()->builder->translation_keys();
		...
		foreach (['blog', 'categories', 'search', 'author'] as $prefix) {
			...
			foreach ($archive_order as $single_archive_component) {
				...
				if (rt_get_akv('read_more_text', $single_archive_component)) {
					$builder_keys[] = [
		...
		foreach (['blog'] as $prefix) {
			...
			foreach ($hero_elements as $single_hero_component) {
				if (
					...
					&&
					rt_get_akv('title', $single_hero_component)
				) {
					$builder_keys[] = [
						'key' => $prefix . '_hero_custom_title',
						'value' => rt_get_akv('title', $single_hero_component)
					];
				}
				...
				if (
					...
					&&
					rt_get_akv('description', $single_hero_component)
				) {
					$builder_keys[] = [
						'key' => $prefix . '_hero_custom_description',
						'value' => rt_get_akv('description', $single_hero_component)
					];
				}
			}
		}
...
if (!function_exists('rt_customizer_has_i18n_plugin')) {
	function rt_customizer_has_i18n_plugin()
	{
...
if (!function_exists('rt_customizer_translate_dynamic')) {
	function rt_customizer_translate_dynamic($text, $key = null)
	{
