...
const getVariables = ({ itemId, panelType }) => ({
	...
	contacts_icon_color: [
		{
			selector: '[data-footer*="type-1"] [data-id="contacts"].ct-footer-contact-info .ct-icon-container',
			variable: 'icon-color',
			type: 'color:default',
			responsive: true,
		},
...
rtEvents.on(
	...
	({
		...
		const selector = [...document.querySelectorAll( '[data-id="contacts"].ct-footer-contact-info > ul' )];
