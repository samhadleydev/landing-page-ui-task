...
rtEvents.on(
	...
	({ itemId, optionId, optionValue, values }) => {
		const el = document.querySelector(`.rt-footer [data-id="${itemId}"]`);
		...
		if (optionId === "socialsLabelVisibility") {
			if (optionValue.desktop || optionValue.tablet || optionValue.mobile) {
				[...el.querySelectorAll("span.rt-label")].map((el) =>
		...
		if (optionId === "socialsType" || optionId === "socialsFillType") {
			const box = el.querySelector(".rt-social-box");
		...
		if (optionId === "socialsIconSize") {
			el.querySelector(".rt-social-box").dataset.size = values.socialsIconSize;
		}

		if (optionId === "footerSocialsColor") {
			el.querySelector(".rt-social-box").dataset.color = optionValue;
		}
		...
		if (optionId === "footer_socials") {
			...
			el.querySelector(".rt-social-box").innerHTML = "";

			optionValue.map(({ id, enabled }) => {
				if (!enabled) return;

				el.querySelector(".rt-social-box").appendChild(
