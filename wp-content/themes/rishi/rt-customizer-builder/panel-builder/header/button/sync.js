	...
	({ itemId, optionId, optionValue }) => {
		...
		if (optionId === 'header_button_type') {
			updateAndSaveEl(selector, (el) => {
				const button = el.querySelector('[class*="rt-button"]')
				button.classList.remove('rt-button', 'rt-button-ghost')

				button.classList.add(
					optionValue === 'type-1' ? 'rt-button' : 'rt-button-ghost'
				)
			})
		}
		...
		if (optionId === 'header_button_text') {
			...
			updateAndSaveEl(selector, (el) => {
				el.querySelector('[class*="rt-button"]').innerHTML = optionValue
			})
		}

		if (optionId === 'header_button_link') {
			updateAndSaveEl(selector, (el) => {
				el.querySelector('[class*="rt-button"]').href = optionValue
			})
		}

		// if (optionId === 'header_button_minwidth') {
		// 	updateAndSaveEl(selector, (el) => {
		// 		el.querySelector('[class*="rt-button"]').style.width = optionValue.desktop + 'px';
		// 	})
		// }

		updateAndSaveEl(selector, (el) => {
			if (el.querySelector('[class*="rt-button"]').innerHTML == '') {
				el.querySelector('[class*="rt-button"]').parentElement.style.visibility = "hidden";
			} else {
				el.querySelector('[class*="rt-button"]').parentElement.style.visibility = "visible";
			}
		})
...
rtEvents.on(
	...
	({ itemId, optionId, optionValue }) => {
		const selector = `.rt-footer [data-id="${itemId}"]`
		...
		if (optionId === 'header_button_size') {
			el.querySelector('[class*="rt-button"]').dataset.size = optionValue
		}
