...
export const handleMenuOptions = ({
	...
	if (
		...
		updateAndSaveEl(selector, (el) => {
			el.dataset.menu = `${values.header_menu_type}${values.header_menu_type === 'type-2'
	...
	if (optionId === 'headerMenuItemsSpacing') {
		rtEvents.trigger('rt:header:update')
		rtEvents.trigger('rt:header:render-frame')
	}
...
rtEvents.on('ct:header:sync:item:menu', (changeDescriptor) => {
