 ...
 */
if (!function_exists('rt_customizer_image_picker_url')) {
	/**
		 * Image picker url 
		 *
		 * @param [type] $path
		 * @return void
		 */
	function rt_customizer_image_picker_url($path)
	{
		return get_template_directory_uri() . '/rt-customizer-builder/src/images/' . $path;
	}
}
 ...
 */
function rt_customizer_collect_options(
	&$result,
	&$options,
	$settings = [],
	$_recursion_data = []
	...
	foreach ($options as $option_id => &$option) {
		if (isset($option['options'])) { // this is a container.
			...
			rt_customizer_collect_options($result, $option, $settings, $_recursion_data);
 ...
 * @return void
 */
function rt_customizer_get_options($path, $pass_inside = [], $relative = true)
{
	if ($relative) {
		$path = get_template_directory() . '/rt-customizer-builder/customizer-settings/' . $path . '.php';
	}
	...
	return apply_filters('rishi:options:retrieve', rt_get_akv(
		'options',
