 ...
 * Post name.
 */
if (!function_exists('rt_customizer_post_name')) {
	/**
	 * Post options names.
	 *
	 * @return void
	 */
	function rt_customizer_post_name()
	{
		return 'ct_options';
	}
}
 ...
 */
if (!function_exists('rt_customizer_group_ordered_keys')) {
	function rt_customizer_group_ordered_keys($choices)
	{
		$result = [];

		foreach ($choices as $group_id => $group_items) {
			$ordered = rt_customizer_ordered_keys(
				$group_items,
				empty($group_id) ? [] : [
					'additional' => [
						'group' => $group_id
 ...
 * @param array $choices key-value pairs.
 */
if (!function_exists('rt_customizer_ordered_keys')) {
	function rt_customizer_ordered_keys($choices, $args = [])
	{
		if (isset($choices[0])) {
			return $choices;
		}
 ...
 */
if (!function_exists('rt_customizer_output_options_panel')) {
	function rt_customizer_output_options_panel($args = [])
	{
		$args = wp_parse_args(
			$args,
			[
		...
		return rt_customizer_html_tag(
			...
			rt_customizer_html_tag(
				...
				[
					'type' => 'text',
					'data-rt-options' => wp_json_encode($args['options']),
					'class' => 'rara-options-panel-storage',
					'value' => wp_json_encode($args['values']),
					'id' => $args['id_prefix'] . rt_customizer_post_name(),
					'name' => $args['name_prefix'] . '[' . rt_customizer_post_name() . ']',
				]
			)
		);
	}
 ...
 */
if (!function_exists('rt_customizer_transform_options')) {
	function rt_customizer_transform_options($options, $current_data = ['transformed-options' => []])
	{
		...
		foreach ($options as $option_id => &$option) {
			if (isset($option['options'])) {
				...
				$new_current_data = rt_customizer_transform_options($option['options']);

				$current_data['transformed-options'][$option_id]['options'] = $new_current_data['transformed-options'];
				...
				$new_current_data = rt_customizer_transform_options($option);

				foreach ($new_current_data['transformed-options']
					as
					$transformed_option_id => $transform_option) {
					if ('__CT_KEYS_ORDER__' !== $transformed_option_id) {
						$current_data['transformed-options'][$transformed_option_id] = $transform_option;
					}
				}
				...
				foreach ($possible_nested_keys as $nested_possible_nested_key) {
				...
				if (
					...
					$new_current_data = rt_customizer_transform_options(
						$option[$possible_nested_key]
					);

					$current_data['transformed-options'][$option_id][$possible_nested_key] = $new_current_data['transformed-options'];
				}
				 ...
				 */
				if ('rt-layers' === $option['type']) {
					foreach ($option['settings'] as $layers_nested_id => $layers_nested_val) {
						$new_current_data = rt_customizer_transform_options(
							$option['settings'][$layers_nested_id]['options']
						);

						$current_data['transformed-options'][$option_id]['settings'][$layers_nested_id]['options'] = $new_current_data['transformed-options'];
					}
				}
			} else {
